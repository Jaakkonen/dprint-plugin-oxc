...
fn comprehensive_sort() {
    let result = sort(
        "text-white text-sm font-medium hover:bg-blue-500 disabled:opacity-50 \
         gap-2 items-center flex py-2 px-4 bg-blue-500 border rounded-lg",
    );
    let parts: Vec<&str> = result.split_whitespace().collect();
    let pos = |name: &str| parts.iter().position(|&c| c == name).unwrap();

    assert!(pos("flex") < pos("items-center"), "{result}");
    assert!(pos("items-center") < pos("gap-2"), "{result}");
    assert!(pos("rounded-lg") < pos("border"), "{result}");
    assert!(pos("border") < pos("bg-blue-500"), "{result}");
    assert!(pos("bg-blue-500") < pos("px-4"), "{result}");
    assert!(pos("py-2") < pos("text-sm"), "{result}");
    assert!(pos("text-white") < pos("hover:bg-blue-500"), "{result}");
    assert!(pos("text-white") < pos("disabled:opacity-50"), "{result}");
}
...
// ---------------------------------------------------------------------------
// Fixture-based tests: validate against prettier-plugin-tailwindcss output
// ---------------------------------------------------------------------------
//
// The fixture file `prettier_fixtures.txt` contains input/expected pairs
// generated by running prettier-plugin-tailwindcss (see generate_fixtures.mjs).
//
// To regenerate after a Tailwind CSS update:
//   cd tailwind-sorter/tests && bun install && bun generate_fixtures.mjs

/// Parse `prettier_fixtures.txt` and return (line_number, input, expected) triples.
fn load_prettier_fixtures() -> Vec<(usize, String, String)> {
    let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("prettier_fixtures.txt");
    ...
    content
        ...
        .filter_map(|(i, line)| {
            ...
            if parts.len() == 2 {
                Some((i + 1, parts[0].to_string(), parts[1].to_string()))
            } else {
                None
            }
        })
...
#[test]
fn prettier_fixture_parity() {
    let fixtures = load_prettier_fixtures();
    assert!(!fixtures.is_empty(), "No fixtures loaded from prettier_fixtures.txt");

    let sorter = ClassSorter::new(SortOptions::default());
    let mut failures = Vec::new();

    for (line_num, input, expected) in &fixtures {
        let actual = sorter.sort_class_string(input.clone());
        if actual != *expected {
