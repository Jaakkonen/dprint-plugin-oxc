...
mod tests {
    ...
    #[test]
    fn lookup_strips_important() {
        assert!(lookup_sort_key("font-bold!").is_some());
        assert!(lookup_sort_key("p-4!").is_some());
    }

    #[test]
    fn lookup_rejects_unknown_font_values() {
        assert!(lookup_sort_key("font-inherit").is_none());
        assert!(lookup_sort_key("font-bold").is_some());
        assert!(lookup_sort_key("font-sans").is_some());
    }
}
